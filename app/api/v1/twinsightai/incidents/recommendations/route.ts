...
async function getIncidentsContainer() {
  return await getContainer(INCIDENTS_DB_NAME!, INCIDENTS_DB_CONTAINER_NAME!, INCIDENTS_PARTITION_KEY, RECOMMENDATIONS_UNIQUE_KEYS);
}
...
export async function GET(req: Request) {
  try {
    ...
    let query = "SELECT * FROM c WHERE c.type='recommendation' AND c.incidentNo=@incidentNo";
...
export async function POST(req: Request) {
  try {
    ...
    if (!data.incidentNo || !data.agentId || !data.recommendation) {
      return NextResponse.json({ error: "incidentNo, agentId and recommendation are required" }, { status: 400 });
    }
...
export async function PATCH(req: Request) {
  try {
    ...
    const { resources } = await container.items
      .query({
        query: "SELECT * FROM c WHERE c.type='recommendation' AND c.incidentNo=@incidentNo AND c.agentId=@agentId",
        parameters: [
    ...
    if (!resources.length) return NextResponse.json({ error: "Recommendation not found" }, { status: 404 });
...
export async function DELETE(req: Request) {
  try {
    ...
    if (incidentNo && !agentId) {
      const { resources } = await container.items
        .query({
          query: "SELECT c.id FROM c WHERE c.type='recommendation' AND c.incidentNo=@incidentNo",
      ...
      return NextResponse.json({ message: `Deleted all recommendations for incident ${incidentNo}` });
    }

    // Delete all recommendations
    const { resources } = await container.items.query("SELECT * FROM c WHERE c.type='recommendation'").fetchAll();
    for (const doc of resources) {
